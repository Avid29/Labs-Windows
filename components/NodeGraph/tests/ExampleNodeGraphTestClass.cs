...
namespace NodeGraphExperiment.Tests;

[TestClass]
public partial class ExampleNodeGraphTestClass : VisualUITestBase
{
    // If you don't need access to UI objects directly or async code, use this pattern.
    [TestMethod]
    public void SimpleSynchronousExampleTest()
    {
        var assembly = typeof(NodeGraph).Assembly;
        var type = assembly.GetType(typeof(NodeGraph).FullName ?? string.Empty);

        Assert.IsNotNull(type, "Could not find NodeGraph type.");
        Assert.AreEqual(typeof(NodeGraph), type, "Type of NodeGraph does not match expected type.");
    }
    ...
    [UIThreadTestMethod]
    public void SimpleUIExamplePageTest(ExampleNodeGraphTestPage page)
    {
        // You can use the Toolkit Visual Tree helpers here to find the component by type or name:
        var component = page.FindDescendant<NodeGraph_ClassicBinding>();

        Assert.IsNotNull(component);

        var componentByName = page.FindDescendant("NodeGraphControl");
    ...
    [UIThreadTestMethod]
    public async Task SimpleAsyncUIExamplePageTest(ExampleNodeGraphTestPage page)
    {
    ...
    public async Task ComplexAsyncUIExampleTest()
    {
        await EnqueueAsync(() =>
        {
            var component = new NodeGraph_ClassicBinding();
            Assert.IsNotNull(component);
        });
    }
