...
namespace ImageAccentColorExtractorExperiment.Tests;

[TestClass]
public partial class ExampleImageAccentColorExtractorTestClass : VisualUITestBase
{
    // If you don't need access to UI objects directly or async code, use this pattern.
    [TestMethod]
    public void SimpleSynchronousExampleTest()
    {
        var assembly = typeof(ImageAccentColorExtractor).Assembly;
        var type = assembly.GetType(typeof(ImageAccentColorExtractor).FullName ?? string.Empty);

        Assert.IsNotNull(type, "Could not find ImageAccentColorExtractor type.");
        Assert.AreEqual(typeof(ImageAccentColorExtractor), type, "Type of ImageAccentColorExtractor does not match expected type.");
    }
    ...
    {
        var component = new ImageAccentColorExtractor();
    ...
    [UIThreadTestMethod]
    public void SimpleUIExamplePageTest(ExampleImageAccentColorExtractorTestPage page)
    {
        // You can use the Toolkit Visual Tree helpers here to find the component by type or name:
        var component = page.FindDescendant<ImageAccentColorExtractor_ClassicBinding>();

        Assert.IsNotNull(component);

        var componentByName = page.FindDescendant("ImageAccentColorExtractorControl");
    ...
    [UIThreadTestMethod]
    public async Task SimpleAsyncUIExamplePageTest(ExampleImageAccentColorExtractorTestPage page)
    {
    ...
    public async Task ComplexAsyncUIExampleTest()
    {
        await EnqueueAsync(() =>
        {
            var component = new ImageAccentColorExtractor_ClassicBinding();
            Assert.IsNotNull(component);
        });
    }
